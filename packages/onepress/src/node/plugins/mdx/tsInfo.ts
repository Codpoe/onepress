...
export function extractInterfaceInfo(
  ...
  if (!fileSymbol || !fileSymbol.exports) {
    throw new Error(`[onepress] Unexpected fileSymbol. ${filePath}`);
  }
  ...
  if (!exportSymbol) {
    throw new Error(
      `[onepress] Named export '${exportName}' is not found in file ${filePath}`
    );
  }
  ...
  if (!sourceDeclaration) {
    throw new Error(
      `[onepress] Can not find sourceDeclaration for ${exportName}`
    );
  }
  ...
  function collectInterfaceInfo(
    ...
    if (!ts.isInterfaceDeclaration(sourceDeclaration)) {
      throw new Error(`[onepress] Target is not an InterfaceDeclaration`);
    }

    if (!sourceSymbol) {
      throw new Error(`[onepress] Can not find symbol`);
    }
    ...
    sourceSymbol.members?.forEach(member => {
      ...
      if (
        ...
        throw new Error(
          `[onepress] Unexpected declaration type in interface. name: ${memberName}, kind: ${
